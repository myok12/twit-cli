...
Twitter.prototype = {
    ...
    retweet: function (id, shouldDelete, fn) {
        if (!id) return fn(new Error("You must provide an id"));
        ...
        if (shouldDelete) {
            return this.remove(id, fn);
        }

        this.api.post("statuses/retweet/" + id, function (err, res) {
            if (err) return fn(new TwitterError(err));
            fn(null, new Tweet(res));
        });
    },

    fav: function (id, shouldDelete, fn) {
        if (!id) return fn(new Error("You must provide an id"));
        ...
        var url = "favorites/";
        url += shouldDelete ? "destroy" : "create";
        this.api.post(url, {id: id}, function (err, res) {
            if (err) return fn(new TwitterError(err));
            fn(null, new Tweet(res));
        });
    }
